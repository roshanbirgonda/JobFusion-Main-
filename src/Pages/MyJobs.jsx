...
const MyJobs = () => {
      ...
      try {
        ...
        fetch("http://localhost:5000/jobs", {
          ...
          .then((data) => {
            ...
            setFilteredJobs(data); // Assuming setFilteredJobs is a function to update state
            setJobs(data); // Assuming setJobs is a function to update state
            setIsLoading(false);
          })
          .catch((error) => {
  ...
  //       console.log(email);
  //       const token = localStorage.getItem("token");
  //       const response = await fetch(`http://localhost:5000/myJobs/${email}`, {
  //         method: "GET",
  //         headers: {
  //           Authorization: `Bearer ${token}`,
  //         },
            ...
            <div className="rounded-t mb-0 px-4 py-3 border-0">
              <div className="flex flex-wrap items-center">
                ...
                <div className="relative w-full px-4 max-w-full flex-grow flex-1 text-right">
                  <Link
                    to="/post-job"
                    className="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 hover:bg-purple-500 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"
                    type="button"
                  >
                    Post a new Job
                  </Link>
                  ...
                  <tbody>
                    {currentJobs.map((job, index) => (
                      <tr key={index}>
                        ...
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-s whitespace-nowrap p-4">
                          <button className="hover:underline duration-500">
                        ...
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                          <button
                            ...
                            className="bg-red-700 py-2 px-6 text-white rounded-sm hover:bg-red-500"
                          >
                            Delete
                          </button>
                        </td>
